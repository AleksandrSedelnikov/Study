#RELEASE 3.0 (количество строчек: 151)
# Изменения по сравнению с release 2.1:
# + Добавлено выполнение скрипта по введённому ключевому слову при вызове (./"name_script" start)
...
# параметр запуска
case $# in # вводится параметр в консоль при вызове скрипта
1) # если параметр один
	raz=$1 # запись введённого параметра в переменную raz
	if [ "$raz" = "start" ] # проверка введён ли параметр start
		then # если верно
			echo "Script startup initialization: successful." # ответ скрипта на верность параметра
		...
		else # если нет или пусто
			echo "Script startup initialization: unsuccessful." # ответ скрипта на неверный параметр или его отсутствие
	fi
;;
*) # остальные случаи
	echo "Script startup initialization: unsuccessful." # ответ скрипта если ничего или больше одного параметра
;;
esac
