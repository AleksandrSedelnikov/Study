#RELEASE 3.1 (количество строчек: 151)
# Изменения по сравнению с release 3.0:
# + Вывод ошибок добавлен только по решению пользователя
# Известные ошибки в скрипте:
# ~ Если выбрано неверное условие продолжения выполнения скрипта, то выдаёт ввод параметров
# ~ Не запускается скрипт при вызове командой sh
...
function script() {
	...
	if [ "$x" != "1" ] && [ "$x" != "2" ] # проверка на количество параметров: 1 или 2 - продолжение выполнения скрипта, 0 или больше 2 - запрещение выполнения скрипта
		...
		"1") # введен один параметр
			...
			then
				...
				then
					echo -n "Введите текст: "; read text # ввод текста 
					if [ "$text" != "" ]
						then
							...
							echo -n "Если да - y, если нет - n"; read resh
							if [ "$resh" == "y" ]
								then
									echo "Введённый Вами текст был добавлен в файл с параметром"
									echo "$text" >> $parametr_1 # добавление введенного текста в конец файла с именем параметра
	...
	echo -n "Хотите продолжить выполнение кода?(y - да, n - нет): "; read usl
	if [ "$usl" == "y" ]
		then
			per1=1
		elif [ "$usl" == "n" ]
...
1)
	...
	if [ "$raz" = "start" ]
	then
        echo "Script startup initialization: successful."
		while [ "$per" != 0 ] # цикл выполнения функции скрипта(пока решение равно 1 - выполняется, если 0 - заканчивает выполнение)
        do
        echo -n "Введите один или два параметра: "; read parametr # вводится(ятся) параметр(ы)
		...
		echo -n "Для вывода ошибок введите - error:" ; read usl1
		if [ "$usl1" = "error" ]
			then
		...
		echo "Script startup initialization: unsuccessful."
	fi
	;;
*)
	echo "Script startup initialization: unsuccessful."
